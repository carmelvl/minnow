...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  if (below > checkpoint) { distance_below = below - checkpoint; } else { distance_below = checkpoint - below; }
  uint64_t same = base; // the same universe
  uint64_t distance_same;
  if (same > checkpoint) { distance_same = same - checkpoint; } else { distance_same = checkpoint - same; }
  uint64_t above = base + WRAP_MOD; // in the above universe
  uint64_t distance_above; 
  if (above > checkpoint) { distance_above = above - checkpoint; } else { distance_above = checkpoint - above; }
