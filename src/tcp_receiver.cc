...
void TCPReceiver::receive( TCPSenderMessage message )
{
  ...
  if (message.SYN) { start += 1; }  // if there is a SYN it actually starts 1 bit later
...
TCPReceiverMessage TCPReceiver::send() const
{
  ...
  if (zero_point) { 
    uint64_t ack = reassembler_.writer().bytes_pushed() + 1; // then our ack is where we are in the bytestream, +1 for SYN
